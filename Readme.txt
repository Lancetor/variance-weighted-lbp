...
LBP方差加权实现原理：
1. 将图像分成 2×2 不重叠块，计算每块方差
2. 以中心块方差为基准，比较邻域 8 个块的方差
3. 邻域方差 > 中心 → +1；邻域方差 < 中心 → -1
4. 权重 = 邻域方差 / 中心方差
最终统计成 LBP 特征向量。

## 加速
预计算 2×2 方差图，8 方向用仿射变换批量平移，避免逐像素循环。

两种统计方式：

| 方式 | 维度 | 组成 | 耗时（70 ROI） |
|---|---|---|---|
| 方式 1 | 767 维 | 正方向 256 + 负方向 255 + 权重图 256 | ~0.2s |
| 方式 2 | 511 维 | 带方向权重图 | 2~11s |

方式 1 用 `gray_histo` 三路批量统计，快；
方式 2 用Tuple算子进行统计，较慢但维度紧凑。

## 说明

只提供方差加权 LBP 实现，不含完整检测程序。
